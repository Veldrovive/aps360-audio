...
class GTZANContrastiveModelLarge_G(nn.Module):
    def __init__(self, contrastive_dim=128):
        super(GTZANContrastiveModelLarge_G, self).__init__()
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.dropout = nn.Dropout(p=0.5)
        self.kernel_size = 3
        ...
        self.projective_head = nn.Linear(1024, contrastive_dim)
        self.name = f"M(GTZANContrastiveModelLarge_G)_CONTDIM({contrastive_dim})_KS({self.kernel_size})"
        self.size = f"Large"
...
class GTZANContrastiveModelXLarge_G(nn.Module):
    def __init__(self, contrastive_dim=128):
        super(GTZANContrastiveModelXLarge_G, self).__init__()
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.dropout = nn.Dropout(p=0.5)
        self.kernel_size = 3
        ...
        self.projective_head = nn.Linear(1024, contrastive_dim)
        self.name = f"M(GTZANContrastiveModelXLarge_G)_CONTDIM({contrastive_dim})_KS({self.kernel_size})"
        self.size = f"XLarge"
